...
class bulk_actions
{
    ...
    {
        // Add bulk actions for generator post type
        add_filter('bulk_actions-edit-generator', [$this, 'register_bulk_actions']);
        add_filter('handle_bulk_actions-edit-generator', [$this, 'handle_bulk_actions'], 10, 3);
    ...
    private function handle_send_target_bulk_action($redirect_to, $post_ids) {
        ...
        $send_target_value = '';
        if (isset($_POST['ppp_gen_send_target_value']) && !empty($_POST['ppp_gen_send_target_value'])) {
            $send_target_value = $_POST['ppp_gen_send_target_value'];
        } elseif (isset($_REQUEST['ppp_gen_send_target_value']) && !empty($_REQUEST['ppp_gen_send_target_value'])) {
            $send_target_value = $_REQUEST['ppp_gen_send_target_value'];
        }
        ...
        foreach ($post_ids as $post_id) {
            ...
            $result = update_field('ppp_gen_send_target', $new_send_target_url, $post_id);
        ...
        set_transient('bulk_action_generator_send_target_updated', $updated_count, 30);
    ...
    private function handle_release_schedule_bulk_action($redirect_to, $post_ids) {
        ...
        $schedule_value = '';
        if (isset($_POST['ppp_gen_release_schedule_value']) && !empty($_POST['ppp_gen_release_schedule_value'])) {
            $schedule_value = $_POST['ppp_gen_release_schedule_value'];
        } elseif (isset($_REQUEST['ppp_gen_release_schedule_value']) && !empty($_REQUEST['ppp_gen_release_schedule_value'])) {
            $schedule_value = $_REQUEST['ppp_gen_release_schedule_value'];
        }
        ...
        foreach ($post_ids as $post_id) {
            // Update the ACF field
            $result = update_field('ppp_gen_release_schedule', $new_schedule, $post_id);
        ...
        // Store the count for the admin notice
        set_transient('bulk_action_generator_release_schedule_updated', $updated_count, 30);
    ...
    public function bulk_action_admin_notice() {
        if (!isset($_REQUEST['post_type']) || $_REQUEST['post_type'] !== 'generator') {
            return;
        }

        // Check for send target updates
        $send_target_updated_count = get_transient('bulk_action_generator_send_target_updated');
        if ($send_target_updated_count !== false) {
            delete_transient('bulk_action_generator_send_target_updated');
            $message = sprintf(
                _n(
                    '%d generator updated with new send target.',
                    '%d generators updated with new send target.',
        ...
        $schedule_updated_count = get_transient('bulk_action_generator_release_schedule_updated');
        if ($schedule_updated_count !== false) {
            delete_transient('bulk_action_generator_release_schedule_updated');
            $message = sprintf(
                _n(
                    '%d generator updated with new schedule.',
                    '%d generators updated with new schedule.',
        ...
        jQuery(document).ready(function($) {
            ...
            $('select[name="action"]').on('change', function() {
                ...
                if (selectedAction === 'change_send_target') {
                    ...
                    formHtml += '<label for="ppp_gen_send_target_value"><strong>New Send Target:</strong></label> ';
                    formHtml += '<select name="ppp_gen_send_target_value" id="ppp_gen_send_target_value" style="margin-left: 10px;" required>';
                    ...
                    formHtml += '<label for="ppp_gen_release_schedule_value"><strong>New Release Schedule:</strong></label> ';
                    formHtml += '<select name="ppp_gen_release_schedule_value" id="ppp_gen_release_schedule_value" style="margin-left: 10px;" required>';
            ...
            $('form#posts-filter').on('submit', function(e) {
                ...
                if (selectedAction === 'change_send_target') {
                    var targetValue = $('#ppp_gen_send_target_value').val();
                    if (!targetValue) {
                    ...
                    var scheduleValue = $('#ppp_gen_release_schedule_value').val();
                    if (!scheduleValue) {
